    ...
    stage('Scan image with twistcli') {
        try {
            withCredentials([usernamePassword(credentialsId: 'twistlock_creds', passwordVariable: 'TL_PASS', usernameVariable: 'TL_USER')]) {
                ...
                sh 'sudo chmod a+x ./twistcli'
                sh "./twistcli images scan --u $TL_USER --p $TL_PASS --address https://$TL_CONSOLE --details harbor-master-test.rbenavente.demo.twistlock.com/library/evilpetclinic:latest"
            }
        } catch (err) {
            echo err.getMessage()
            echo "Error detected"
